...
A file is opened by \Robject{H5Fopen}. 
...
The \Robject{\$} and \Robject{\&} operators can be used to access the next group level. While the \Robject{\$} operator reads the object from disk, the \Robject{\&} operator returns a group or dataset handle.
...
Both of the following code lines return the matrix \Robject{C}. \warning{However, while first version reads the whole tree \Robject{/foo} in memory and then subsets to \Robject{/foobaa/C}, the second version only reads the matrix \Robject{C}. The first "\$" in "h5f\$foo\$foobaa\$C" reads the dataset, the other "\$" are accessors of a list. Remind that this can have severe consequences for large datasets and datastructures.}
...
The same works as well for writing to datasets. \warning{Remind that it is only guarenteed that the data is written on disk after a call to \Robject{H5Fflush} or after closing of the file.}
<<accessorB2>>=
h5d[3,,] = 1:4
H5Fflush(h5f)
@
...
\subsection{Large integer data types}
