...
(>= 4.0.0) of **R** (www.r-project.org). After installing **R** you can
...
The chosen chunk size and compression level have a strong impact on the reading
...
We can see that the **df** entry has now disappeared from the listing.  In most cases, if you have a hierarchy within the file, `h5delete()`  will remove children of the deleted entry too.  In this example we remove **foo** and the datasets below it are deleted too.  Notice too that the size of the file as decreased.
