...
/* hid_t H5Gcreate( hid_t loc_id, const char *name, hid_t lcpl_id, hid_t
...
SEXP _H5Gget_info(SEXP _group_id) {
  ...
  if (herr < 0) {
    error("Error getting group information");
  }
...
SEXP _H5Gget_info_by_name(SEXP _loc_id, SEXP _group_name) {
  ...
  H5G_info_t group_info;
  herr_t herr =
      H5Gget_info_by_name(loc_id, group_name, &group_info, H5P_DEFAULT);
  if (herr < 0) {
    error("Error getting group information");
  }
 ...
 * index_type, H5_iter_order_t order, hsize_t n, H5G_info_t *group_info, hid_t
 * lapl_id ) */
/* TODO more parameters:  hid_t lapl_id */
SEXP _H5Gget_info_by_idx(SEXP _loc_id, SEXP _group_name, SEXP _index_type,
                         SEXP _order, SEXP _n) {
  ...
  H5G_info_t group_info;
  herr_t herr = H5Gget_info_by_idx(loc_id, group_name, index_type, order, n,
