...
SEXP _h5createDataFrame(SEXP _obj, SEXP _loc_id, SEXP _name, SEXP _level,
                        SEXP _chunk) {
  ...
  hid_t dset_id =
      H5Dcreate(loc_id, name, tid, space, H5P_DEFAULT, plist, H5P_DEFAULT);
  ...
  for (R_xlen_t i = 0; i < LENGTH(_obj); i++) {
    ...
    if (TYPEOF(VECTOR_ELT(_obj, i)) == INTSXP) {
      ...
      for (k = 0; k < XLENGTH(VECTOR_ELT(_obj, i)); k++) {
        for (j = 0; (j < LENGTH(STRING_ELT(VECTOR_ELT(_obj, i), k))) &&
                    (j < (strsize[i]));
             j++) {
