...
herr_t custom_print_cb(unsigned n, const H5E_error2_t *err_desc,
  ...
  if (cd->n >= 400) {
    ...
    snprintf(cd->txt[cd->n], 1024, "%*serror #%03u: %s in %s(): line %u",
             indent, "", n, err_desc->file_name, err_desc->func_name,
             err_desc->line);
...
herr_t custom_print_cb_compact(unsigned n, const H5E_error2_t *err_desc,
                               void *client_data) {
  char maj[MSG_SIZE];
  char min[MSG_SIZE];
  char cls[MSG_SIZE];
...
herr_t _rhdf5PrintErrorR(hid_t estack_id, void *stream) {
  ...
  if (s > 0) {
    ...
    client_data.n = 0;
    herr_t eee = H5Ewalk(estack_id_copy, H5E_WALK_UPWARD, &custom_print_cb,
                         &client_data);
    if (eee < 0) {
      error("Error walking through HDF5 error stack");
    }
...
herr_t _rhdf5PrintErrorRcompact(hid_t estack_id, void *stream) {
  ...
  if (s > 0) {
    ...
    struct DataCollector client_data;
    client_data.n = 0;
    herr_t eee = H5Ewalk(estack_id_copy, H5E_WALK_DOWNWARD,
                         &custom_print_cb_compact, &client_data);
    if (eee < 0) {
      error("Error walking through HDF5 error stack");
    }
