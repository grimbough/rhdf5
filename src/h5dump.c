...
herr_t opAddToDumpTree(hid_t g_id, const char *name, const H5L_info2_t *info,
                       void *op_data) {
    ...
    if (data->n > 0 &&
      ...
      if ((data->maxdepth < 0) || (data->depth < data->maxdepth)) {
        ...
        herr = H5Literate_by_name2(g_id, name, H5_INDEX_NAME, data->order, NULL,
                                   opAddToDumpTree, (void *)data, H5P_DEFAULT);
        data->depth = data->depth - 1;
        data->insertAsChild = 0;
        data->last = last;
        data->group = group;
...
SEXP getTree(opObjListElement *elstart, opDumpTree *data, hid_t loc_id,
             int depth) {
