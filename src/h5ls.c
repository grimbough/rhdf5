...
herr_t opAddToObjList(hid_t g_id, const char *name, const H5L_info2_t *info,
                      void *op_data) {
  ...
  struct opObjListElement *newElement =
      (opObjListElement *)R_alloc(1, sizeof(struct opObjListElement));
  newElement->idx = data->n;
  newElement->name = (char *)R_alloc(1, (strlen(name) + 1) * sizeof(char));
  strcpy(newElement->name, name);
  newElement->group =
      (char *)R_alloc(1, (strlen(data->group) + 1) * sizeof(char));
  ...
  case H5O_TYPE_GROUP: {
    ...
    if (data->n > 0 &&
        group_check(data->last, infobuf.token, infobuf.fileno, g_id)) {
      ...
      if ((data->maxdepth < 0) || (data->depth < data->maxdepth)) {
        ...
        data->group =
            (char *)R_alloc((strlen(name) + strlen(group) + 2), sizeof(char));
        ...
        herr = H5Literate_by_name2(g_id, name, H5_INDEX_NAME, data->order, NULL,
                                   opAddToObjList, (void *)data, H5P_DEFAULT);
        data->depth = data->depth - 1;
        data->group = group;
      }
    }
...
SEXP _h5ls(SEXP _loc_id, SEXP _depth, SEXP _datasetinfo, SEXP _index_type,
           SEXP _order, SEXP _native) {
  ...
  herr_t herr = H5Literate2(loc_id, data.index_type, data.order, &idx,
                            &opAddToObjList, &data);
  ...
  if (herr < 0) {
    ...
    const char *nms[] = {"group",     "name",   "ltype", "cset",  "otype",
                         "num_attrs", "dclass", "dtype", "stype", "rank",
                         "dim",       "maxdim", ""};
