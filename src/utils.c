...
void concatdim(char *s1, hsize_t next_dim, int index) {
  ...
  snprintf(s1, 1000, "%.977s%" PRIu64 "%.3s", tmp, (uint64_t)next_dim,
           index ? " x " : "");
}
...
void concatdim_native(char *s1, hsize_t next_dim, int index) {
  ...
  snprintf(s1, 1000, "%.977s%.3s%" PRIu64, tmp, index ? " x " : "",
           (uint64_t)next_dim);
}

void format_dimensions(H5S_class_t space_type, opObjListElement *newElement,
                       hsize_t *size, hsize_t *maxsize, bool native) {
 ...
 ************************************************************/
int group_check(struct opObjListElement *od, H5O_token_t target_token,
                unsigned long target_fileno, hid_t loc_id) {
  int cmp_result;
