...
void uint32_to_int32(void *intbuf, hsize_t n, void *buf) {
  ...
  if (warn) {
    warning("NAs produced by integer overflow while converting unsigned 32-bit "
            "integer from HDF5 to a signed 32-bit integer in R.\nChoose "
            "bit64conversion='bit64' or bit64conversion='double' to avoid data "
            "loss");
  }
}
...
void int64_to_int32(void *intbuf, hsize_t n, void *buf, H5T_sign_t sign) {
  ...
  if (warn) {
    warning(
        "NAs produced by integer overflow while converting 64-bit integer from "
        "HDF5 to a 32-bit integer in R.\nChoose bit64conversion='bit64' or "
        "bit64conversion='double' to avoid data loss");
  }
}
...
void int64_to_double(void *intbuf, hsize_t n, void *buf, H5T_sign_t sign) {
  ...
  if (warn_double) {
    warning(
        "integer precision lost while converting 64-bit integer from HDF5 to "
        "double in R.\nChoose bit64conversion='bit64' to avoid data loss and "
        "see the vignette 'rhdf5' for more details about 64-bit integers.");
  }
}
...
void int64_to_integer64(void *intbuf, hsize_t n, void *buf, H5T_sign_t sign) {
  ...
  if (warn_overflow_64bit) {
    warning("NAs produced by integer overflow while converting unsigned 64-bit "
            "integer from HDF5 to signed 64-bit integer in R.");
  }
}
