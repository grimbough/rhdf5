...
SEXP _H5Sget_simple_extent_dims(SEXP _space_id) {
  ...
  if (rank < 0) {
    ...
    for (int i = 0; i < rank; i++) {
      ...
      maxsize_is_numeric +=
          (maxsize[i] > R_LEN_T_MAX) && (maxsize[i] != H5S_UNLIMITED);
    }
    if (size_is_numeric) {
    ...
    if (maxsize_is_numeric) {
      ...
      for (int i = 0; i < rank; i++) {
        REAL(Rmaxsize)
        [i] = (maxsize[i] == H5S_UNLIMITED) ? -1 : (double)maxsize[i];
      }
    } else {
      Rmaxsize = PROTECT(allocVector(INTSXP, rank));
      for (int i = 0; i < rank; i++) {
        INTEGER(Rmaxsize)
        [i] = (maxsize[i] == H5S_UNLIMITED) ? -1 : (int)maxsize[i];
      }
    }
  }
...
/* herr_t H5Sset_extent_simple( hid_t space_id, int rank, const hsize_t
 * *current_size, const hsize_t *maximum_size ) */
SEXP _H5Sset_extent_simple(SEXP _space_id, SEXP _current_size,
                           SEXP _maximum_size) {
  hid_t space_id = STRSXP_2_HID(_space_id);
  hid_t herr;
...
SEXP _H5Sget_select_hyper_blocklist(SEXP _space_id, SEXP _startblock,
                                    SEXP _numblocks, SEXP _bufferlength) {
  ...
  herr_t herr =
      H5Sget_select_hyper_blocklist(space_id, startblock, numblocks, buf);
  if (herr < 0) {
 ...
 */
SEXP _H5Sselect_hyperslab(SEXP _space_id, SEXP _op, SEXP _start, SEXP _stride,
                          SEXP _count, SEXP _block) {
  hid_t space_id = STRSXP_2_HID(_space_id);
...
/* hid_t  H5Scombine_hyperslab ( hid_t space_id, H5S_seloper_t op, const hsize_t
 start[], const hsize_t stride[], const hsize_t count[], const hsize_t block[] )
 */
SEXP _H5Scombine_hyperslab(SEXP _space_id, SEXP _op, SEXP _start, SEXP _stride,
                           SEXP _count, SEXP _block) {
  hid_t space_id = STRSXP_2_HID(_space_id);
  H5S_seloper_t op = (H5S_seloper_t)INTEGER(_op)[0];
  ...
  hid_t new_space_id =
      H5Scombine_hyperslab(space_id, op, start, stride, count, block);
...
/* herr_t H5Sselect_elements(hid_t space_id, H5S_seloper_t op, size_t
 * num_elements, const hsize_t *coord); */
SEXP _H5Sselect_elements(SEXP _space_id, SEXP _op, SEXP _num_elements,
                         SEXP _coord) {
