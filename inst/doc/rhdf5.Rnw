...
By setting {\bf bit64conversion='int'}, a coercing to 32-bit integers
is enforced, with the risc of data loss, but with the insurance that
...
they are not represented as integer values anymore. For larger numbers there is still a data loss.
