...
\begin{center}
  \includegraphics[width=0.75\textwidth]{../demoChunkSize/chunksize.pdf}
\end{center}
...
may run into troubles. rhdf5 is able to deal with 64-bit integers, but
you still should pay attention when dealing with 64-bit integers. 
...
d = h5createDataset(file="newfile3.h5", dataset="D64", dims=c(10,5,2),H5type="H5T_NATIVE_INT64")
h5write(D,file="newfile3.h5",name="D64")
@

There are three different ways of reading 64-bit integers in
R. \code{H5Dread} and \code{h5read} have the argument
\code{bit64conversion} the specify the conversion method.

By setting {\bf bit64conversion='int'}, a coercing to 32-bit integers is enforced, with the risc of data loss, but with the insurance that numbers are represented as integers.
...
{\bf bit64conversion='double'} coerces the 64-bit integers to floating
point numbers. doubles can represent integers with up to 54-bits, but
they are not represented as integer values anymore. For larger numbers there is again a data loss.
...
{\bf bit64conversion='bit64'} is recommended way of coercing. It
represents the 64-bit integers as objects of class 'integer64' as
defined in the package 'bit64'. Make sure that you have installed
'bit64'. The datatype 'integer64' is not part of base R, but defined
in an external package. This can produce unexpected behaviour when
working with the data. When choosing this option the library bit64
will be loaded.
