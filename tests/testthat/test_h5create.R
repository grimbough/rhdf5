...
test_that("Don't overwrite existing file", {
  expect_message(h5createFile(file = h5File), regexp = "already exists.\n$")
})
...
test_that("Invalid inputs", {
  expect_error(
    suppressWarnings(h5createDataset(
      file = h5File,
      dataset = "fail",
      dims = "twenty"
    )),
  ...
  expect_warning(h5createDataset(
    file = h5File,
    dataset = "fail",
    dims = c(10, 20),
    level = 1,
...
test_that("Fail is attribute already exists", {
  expect_false(h5createAttribute(
    obj = "foo",
    file = h5File,
    attr = "foo_attr",
    dims = c(1, 1)
...
test_that("Fail if dims or maxdims not numeric", {
  expect_error(h5createAttribute(
    obj = "foo",
    file = h5File,
    attr = "foo_attr2",
    dims = "A"
...
test_that("string encoding is handled properly", {
  ...
  # Now Unicode.
  expect_silent(h5createAttribute(
    file = h5File,
    obj = "foo",
    dims = c(1, 1),
    attr = "utf_str_attr",
