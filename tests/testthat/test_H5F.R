...
test_that("Invalid file names", {
  expect_error(
    H5Fcreate(name = 1),
    regexp = "must be a character string of length 1",
    fixed = TRUE
  )
  expect_error(
    H5Fcreate(name = c("F1", "F2")),
    regexp = "must be a character string of length 1",
    fixed = TRUE
  )
})
...
test_that("H5Fget_name", {
  expect_match(H5Fget_name(h5obj = fid), basename(h5File), fixed = TRUE)
})

test_that("H5Fget_filesize", {
  expect_type(H5Fget_filesize(h5file = fid), "double") |>
    expect_lt(1000) |>
    expect_gt(0)
})
