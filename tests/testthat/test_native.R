...
test_that("h5read/write supports native", {
  ...
  do_matrix <- function(values, nrow, ncol, val) {
    ...
    h5write(
      m0,
      file = h5File,
      name = paste0("test/nonnative", val),
      native = FALSE
    )
    m1 <- h5read(
      file = h5File,
  ...
  do_array <- function(values, dim, val) {
    ...
    h5write(
      m0,
      file = h5File,
      name = paste0("test/nonnative-array", val),
      native = FALSE
    )
    m1 <- h5read(
      file = h5File,
    ...
    h5write(
      m0,
      file = h5File,
      name = paste0("test/native-array", val),
      native = TRUE
    )
    m1 <- h5read(
      file = h5File,
...
test_that("h5read/h5write supports index and hyperslab", {
  ...
  expect_equal(
    h5read(h5File, "D", native = TRUE)[2:5, 2:4],
    matrix(1:12, nrow = 4, ncol = 3)
  )
  res <- h5read(
    file = h5File,
  ...
  expect_equal(
    h5read(h5File, "D", native = TRUE)[c(1:3, 5:7), c(1:4, 6:9)],
    matrix(1:48, nrow = 6, ncol = 8)
  )
  res <- h5read(
    file = h5File,
...
test_that("H5F native functionality", {
  for (native in c(FALSE, TRUE)) {
    ...
    expect_equivalent(
      h5read(h5File, paste0("H5Ftest/A", native), native = native),
      B
    )
  }
})
