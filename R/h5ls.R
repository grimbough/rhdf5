...
h5ls <- function(
  ...
  if (length(datasetinfo) != 1 || !is.logical(datasetinfo)) {
    stop("'datasetinfo' must be a logical of length 1")
  }
  ...
  if (is.logical(recursive)) {
    ...
    if (length(recursive) > 1) {
      ...
      stop(
        "value 0 for 'recursive' is undefined, either a positive integer or negative (maximum recursion)"
      )
    }
  } else {
