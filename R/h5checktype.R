h5checktype <- function(
  h5id,
  type,
  fctname = deparse(match.call()[1]),
  allow.character = FALSE
) {
  if (!is(h5id, "H5IdComponent")) {
    if (allow.character) {
      stop(
        "Error in ",
        fctname,
        ". Argument neither of class H5IdComponent nor a character.",
        call. = FALSE
      )
    } else {
      stop(
  ...
  if (is.character(file)) {
    ...
    if (file.exists(file)) {
      if (h5FileIsOpen(file)) {
        warning(
          "An open HDF5 file handle exists. ",
  ...
  if (is.character(obj)) {
    if (missing(file)) {
      stop(
        "Error in ",
        fctname,
        ". If the object is specified by name, then you have to specify the file."
      )
    }
