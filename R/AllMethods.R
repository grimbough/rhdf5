...
setMethod("show", signature = "H5IdComponent", function(object) {
  ...
  cat(sprintf("HDF5 %s", res$type), if (object@native) "(native)", "\n")
...
#'
#' @export
setMethod(`&`, signature = c("H5IdComponent", "character"), function(e1, e2) {
  H5Oopen(e1, e2)
})
...
#'
#' @export
setMethod("show", signature = "H5Ref", definition = function(object) {
  cat("HDF5 REFERENCE\n")
...
#'
#' @export
setMethod("length", signature = "H5Ref", definition = function(x) {
...
#'
#' @export
setMethod(f = "c", signature = "H5Ref", definition = function(x, ...) {
  elements <- list(x, ...)
...
#'
#' @export
setMethod(f = "[", signature = c("H5Ref", "ANY"), definition = function(x, i) {
  if (h5const2String("H5R_TYPE", x@type) == "H5R_OBJECT") {
    div <- 8L
  } else {
    div <- 12L
