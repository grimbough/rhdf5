...
h5writeDataset.array <- function(obj, h5loc, name, index = NULL, start=NULL, stride=NULL, block=NULL, count=NULL, size=NULL, level=7, native=FALSE) {
  if (is.null(dim(obj))) {
    dim(obj) = length(obj)
  }
  exists <- try( { H5Lexists(h5loc, name) } )
  if (!exists) {
    if (is.null(size)) {
      ...
      temp_dim <- dim(obj)
      if (native) temp_dim <- rev(temp_dim)
