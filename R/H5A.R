...
H5Aopen <- function( h5obj, name ) {
  ...
  if (aid > 0) {
    h5attribute = new("H5IdComponent", ID = aid, native = .native(h5obj))
  } else {
...
H5Aopen_by_name <- function( h5obj, objname = ".", name ) {
  ...
  if (aid > 0) {
    h5attribute = new("H5IdComponent", ID = aid, native = .native(h5ob))
  } else {
    message("HDF5: unable to open attribute")
    h5attribute = FALSE
  }
...
H5Aget_space <- function( h5attribute ) {
  ...
  if (sid > 0) {
    h5space = new("H5IdComponent", ID = sid, native = .native(h5attribute))
  } else {
